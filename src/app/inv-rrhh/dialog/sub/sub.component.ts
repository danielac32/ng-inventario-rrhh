...
constructor(
    public dialogRef: MatDialogRef<SubComponent>,
...
onSubmit() {
      ...
      const {descripcion,stock} = this.updateProduct.value;
      if(!stock) return;
      if(Number(stock)<0){
      	    let message:Message={
 	  ...
 	  if(Number(stock)>this.actualStock){
      	    let message:Message={
	            title:"El numero no puede ser mayor al stock actual",
	            error:true,
	            enable:true,
	            type:2,
	        };
      ...
      this.invService.subStockProduct(this.data?.id,{ 	
      													"stock":Number(stock),
      													"cantidad":Number(finalStock),
      													"observacion":descripcion,
      													"tipo":TipoModificacion.RESTA,
      													"entregado":"NO"
      												}).subscribe((response) => {
